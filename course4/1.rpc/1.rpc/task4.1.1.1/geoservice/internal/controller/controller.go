...
type Controller struct {
	...
	RpcServicer service.GeoRPC
}

func NewController(token *jwtauth.JWTAuth, options ...ControllerOption) *Controller {
	srvc, err := service.NewGeoService(service.WithToken(token))
	if err != nil {
		fmt.Println(err)
	}
	rpcService := service.NewGeoRPCService()
	if err != nil {
		fmt.Println(err)
	}
...
func (c *Controller) SearchAnswer(w http.ResponseWriter, r *http.Request) {
	...
	status, address, err := c.RpcServicer.SearchAnswer(coordinates)
	if err != nil {
		newErrorResponce(w, err, status)
		return
	}
...
func (c *Controller) GeocodeAnswer(w http.ResponseWriter, r *http.Request) {
	...
	status, coords, err := c.RpcServicer.GeocodeAnswer(address)
	if err != nil {
		newErrorResponce(w, err, status)
		return
	}
